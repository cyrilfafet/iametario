...
function LivraisonInner() {
  ...
  const sendAvis = async () => {
    ...
    await fetch("/api/avis", {
      ...
      body: JSON.stringify({ code: livraison.code, stars: selectedStar, nom: avisNom, commentaire: avisCommentaire.trim() || null, montant: 30 + (livraison.solde || 0) }),
    });
              ...
              {selectedStar > 0 && (
                <div className="flex flex-col gap-3">
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="bg-white border border-zinc-200 rounded-xl px-5 py-3 text-sm text-zinc-900 placeholder-zinc-400 focus:outline-none focus:border-blue-500 transition-colors resize-none"
                  />
                  <button
                    onClick={sendAvis}
